 ...
 * 1. (需求) 标题栏增加版本号 "V1.3"。
 * 2. (需求) 左侧边栏底部添加作者信息。
 * 3. (V1.3) 修复了日志自动滚动 Bug。
 * 4. (V1.3) 实现了日志图片显示和菜单栏隐藏。
 */
...
function App() {
  ...
  const [log, setLog] = useState<LogEntry[]>([
    // (需求) 更新版本号
    { type: 'text', content: "欢迎使用 Excel 转换工具 (V1.3)\n" }
  ]);
  ...
  const DraggableField = ({ name, isMapped }: { name: string, isMapped: boolean }) => {
    ...
    return (
      <li ref={drag} className={`flex items-center justify-between p-3 bg-white rounded-md border shadow-sm ${isMapped ? 'opacity-50' : 'cursor-grab'} ${isDragging ? 'opacity-75' : ''}`}>
        ...
        <svg className="w-5 h-5 text-gray-400" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
      </li>
    );
  };
    ...
    if (isMapped) {
      return (
        <li className="flex items-center p-3 bg-green-50 rounded-md border border-green-300 shadow-sm">
          ...
          <svg onClick={() => deleteMapping(mappedSource!)} className="w-5 h-5 text-green-600 cursor-pointer" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
        </li>
      );
    }
    return (
          ...
          <div className="bg-white w-full max-w-md rounded-lg shadow-xl transform transition-all duration-200 scale-100 opacity-100">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <div className="flex items-center">
                    <svg className="w-6 h-6 text-red-600 mr-3" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.01" /></svg>
                ...
                <button onClick={() => setIsModalVisible(false)} className="text-gray-400 hover:text-gray-600">
                    <svg className="w-6 h-6" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
            </div>
            {/* 模态框内容 */}
            <div className="p-6">
      ...
      <div className="flex flex-col h-screen font-sans">
        ...
        <header className="relative w-full h-[60px] flex text-white overflow-hidden z-10">
            ...
            <div className="relative z-10 flex items-center w-full">
                {/* --- (需求 1) 标题和版本号 --- */}
                <div className="w-80 flex-shrink-0 px-6 flex items-baseline">
                    <div>
                      <span className="text-base font-medium">Excel 数据转换工具</span>
                      <span className="text-xs font-normal text-gray-400 ml-2">V1.3</span>
                      <div className="text-xs text-gray-300">by 成都兴城融晟科技有限公司 张潇</div>
                    </div>
                </div>
                {/* --- Tab 栏 --- */}
                ...
                <div className="mb-4">
                  ...
                  {fileA ? (
                    <div className="flex items-center p-3 rounded-lg border-2 border-green-500 bg-green-50 text-green-800">
                      <svg className="w-5 h-5 mr-2 flex-shrink-0" xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
